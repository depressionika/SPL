...
posts = response.json() #преобразуем ответ от API в формат JSON (список постов).
posts_users = [post for post in posts if post['userId'] % 2 == 0] #берем только четные userid
...
rows = cursor.fetchall() #возвращаем все строки, найденные по запросу
