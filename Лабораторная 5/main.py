import sys
from PyQt5.QtWidgets import QApplication, QMainWindow, QTableView, QLineEdit, QPushButton, QVBoxLayout, QWidget, QLabel, QSizePolicy, QProgressBar
    ...
    async def save_to_db(self, posts):
        try:
            async with aiosqlite.connect('posts.db') as db:
                await db.execute('CREATE TABLE IF NOT EXISTS posts (id INTEGER PRIMARY KEY, user_id INTEGER NOT NULL, title TEXT NOT NULL, body TEXT NOT NULL)')
                total_posts = len(posts)
                for index, post in enumerate(posts):
                    await db.execute('INSERT INTO posts (user_id, title, body) VALUES (?, ?, ?)', (post['userId'], post['title'], post['body']))
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.setGeometry(500, 200, 460, 600)
